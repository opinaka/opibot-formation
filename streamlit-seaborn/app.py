...
# Config Streamlit
st.set_page_config(page_title="Visualisation CSV avec Seaborn", layout="wide")
st.title("📈 Visualisation CSV avec Seaborn")
...
if not csv_files:
    ...
    if not numeric_cols:
        ...
        # Crée le graphique avec Seaborn uniquement
        fig = sns.lineplot(data=df, x="Date", y=selected_col).get_figure()
        fig.tight_layout()
        st.pyplot(fig)  # Streamlit affiche la figure Seaborn
        fig.clf()  # Nettoie la figure
